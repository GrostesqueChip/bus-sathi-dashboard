...
function getRouteColor(properties: Record<string, unknown>, isSelected: boolean) {
  ...
  const newRouteId = String(properties.New_Route_ID || '');

  // SSCL e-bus backbone → purple
  if (newRouteId.startsWith('SSCL-')) return '#7c3aed';
  if (action === 'UPGRADED_TO_TRUNK') return '#059669';
  if (priority === 'HP') return '#2563eb';
...
export default function KashmirNetworkMap({ selectedRouteId }: KashmirNetworkMapProps) {
  ...
  useEffect(() => {
    ...
    fetch('/route-rationalization-kashmir/Rationalised_Routes_Kashmir_v3.geojson')
      .then((response) => {
        if (!response.ok) {
          throw new Error('Failed to load Kashmir route network');
        }
      ...
      .catch((fetchError) => {
        if (isMounted) {
          setError(fetchError instanceof Error ? fetchError.message : 'Unable to load the Kashmir route network');
        }
      });
  ...
  const routeStyle = useMemo(() => {
    return (feature: any) => {
      ...
      return {
        ...
        weight: isSelected ? 7 : isSscl ? 5.5 : action === 'UPGRADED_TO_TRUNK' ? 5 : priority === 'HP' ? 4.5 : 3.5,
  ...
  if (error) {
    return (
      <div className="flex h-full items-center justify-center bg-slate-50 text-center">
        <div>
          <p className="text-base font-black text-slate-800">Kashmir map unavailable</p>
    ...
    return (
      <div className="flex h-full items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm font-black uppercase tracking-[0.18em] text-emerald-700">Loading Kashmir network map</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <MapContainer center={[34.08, 74.80]} zoom={11} className="h-full w-full" zoomControl={false}>
      <TileLayer
      ...
      <GeoJSON
        key={selectedRouteId || 'kashmir-all-routes'}
        data={geoJsonData}
        style={routeStyle}
        onEachFeature={(feature: any, layer) => {
          ...
          const isTourist = Boolean(properties.Tourist_Corridor);

          layer.bindPopup(`
            <div style="min-width: 260px; padding: 8px 5px;">
              <div style="display:flex; justify-content:space-between; gap:12px; margin-bottom:10px;">
                <div>
                  <div style="font-size:18px; font-weight:900; color:#0f172a;">${routeCode !== routeId ? routeCode : routeId}</div>
                ...
                <span style="height:fit-content; padding:6px 10px; border-radius:999px; font-size:10px; font-weight:900; letter-spacing:0.12em; text-transform:uppercase; background:${isSscl ? '#ede9fe' : isTrunk ? '#d1fae5' : '#ccfbf1'}; color:${isSscl ? '#5b21b6' : isTrunk ? '#065f46' : '#0f766e'};">
                  ${isSscl ? 'SSCL backbone' : formatAction(properties.Action_Taken)}
                </span>
              </div>
              ...
              ${
                isTourist
                  ? '<div style="margin-top:6px; padding:8px 10px; border-radius:12px; background:#ecfdf5; color:#065f46; font-size:12px; font-weight:800;">Tourist corridor</div>'
                  : ''
              }
              <a href="/route-rationalization-kashmir/route_maps_kashmir/${routeId}.html" target="_blank" rel="noreferrer" style="display:inline-block; margin-top:12px; color:#059669; font-size:12px; font-weight:900; text-decoration:none;">
