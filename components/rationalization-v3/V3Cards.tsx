...
import type { V3CountItem } from '@/lib/routeRationalizationV3';
...
export function getActionPillClass(action: string) {
  if (action === 'UPGRADED_TO_TRUNK') return 'bg-blue-100 text-blue-800';
...
export function getPriorityPillClass(priority: string) {
  ...
  if (priority === 'MP') return 'bg-cyan-100 text-cyan-800';
...
export function KpiCard({
  ...
  detail: string;
  icon: LucideIcon;
  tone: 'slate' | 'blue' | 'teal' | 'orange' | 'violet';
}) {
  const toneClass = {
...
export function PolicyCard({
  ...
  return (
    <div className="rounded-[1.8rem] border border-white/12 bg-white/9 p-5 backdrop-blur">
      <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-cyan-100">
        <Icon size={15} /> v3 policy
      </p>
